...
(function(d, f, b) {
	...
	a.util = {
		...
		getNullmsg: function(o) {
			...
			l = n.siblings(l).eq(0).text() || n.siblings().find(l).eq(0).text() || n.parent().siblings(l).eq(0).text() || n.parent()
				.siblings().find(l).eq(0).text();
	...
	function h(n, m) {
		var l = (d(window).width() - n.outerWidth()) / 2,
			...
			k = (document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + (k > 0 ?
				k : 0);
		n.css({
			left: l
		}).animate({
	...
	function c() {
		...
		j = d('<div id="Validform_msg"><div class="Validform_title">' + e.tit +
